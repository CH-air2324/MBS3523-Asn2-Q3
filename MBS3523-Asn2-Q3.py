    ...
    if len(indexes) > 0:
        for i in indexes.flatten():
            ...
            if label == 'apple':
                ...
                cv2.putText(img,label +" "+ "["+str(ap)+"]  " + str(round(float(confidence)*100,2)) + '%', (x,y+20),font,2,(0,0,0),2)
            if label == 'banana':
                ba = ba+1
                cv2.rectangle(img,(x,y),(x+w,y+h),color,2)
                cv2.putText(img,label +" "+ "["+str(ba)+"]  " + str(round(float(confidence)*100,2))+ '%', (x,y+20),font,2,(0,0,0),2)
            if label == 'orange':
                og = og+1
                cv2.rectangle(img,(x,y),(x+w,y+h),color,2)
                cv2.putText(img,label +" "+ "["+str(og)+"]  " + str(round(float(confidence)*100,2)) + '%', (x,y+20),font,2,(0,0,0),2)

        cv2.putText(img, "There are: " + str(ap) + " " + 'apple', (1000, 20), font, 1.5, (0, 0, 255), 2)
        cv2.putText(img, "There are: " + str(ba) + " " + 'banana', (1000, 40), font, 1.5, (0, 255, 255), 2)
        cv2.putText(img, "There are: " + str(og) + " " + 'orange', (1000, 60), font, 1.5, (9, 9, 70), 2)
        apt = ""
        bat = ""
        ogt = ""
        Ft = ""
        ...
        if og>0:
            if ap > 0 or ba > 0:
                Ft+="+"
            Ft+= str(og)+'x3'
        cv2.putText(img, "total $: " + Ft + '='+ str(round(ap*1+ba*2+og*3)), (1000, 80), font, 1.5, (0, 0, 0), 2)
